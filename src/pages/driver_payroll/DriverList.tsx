...
export const DriverList = ({ title }: Readonly<PropsTitle>) => {
    ...
    const handleActiveToggle = async (payroll: Driver, active: boolean) => {
        ...
        const driverResp = await DriverApi.getDriver(payroll.driver_code ?? 0);
        if (!isAxiosError(driverResp) && driverResp) {
            const updatedDriverList = driverList.map((item) =>
                item.driver_code !== payroll.driver_code ? item : driverResp,
            );
        ...
        const driverName = `${payroll.driver_name ?? ""} ${payroll.driver_surname ?? ""}`;
        showToastSuccess(
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            <Button
                                                component={Link}
                                                to={`/drivers/${driver.driver_code ?? 0}`}
                                                variant="contained"
                                                color="info"
                                                sx={{
                                                    width: "100%",
