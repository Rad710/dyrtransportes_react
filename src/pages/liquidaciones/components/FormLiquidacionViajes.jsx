...
function FormLiquidacionViajes({
        ...
        const queryLiquidaciones = async () => {
            ...
            if (!result?.response && !result?.message) {
                const formattedResult = result.map(
                    (liquidacion) => new Date(liquidacion.fechaLiquidacion)
                );
    ...
    const handleEditar = () => {
        ...
        const newFormData = {
            id: toEdit.id,
            fechaCreacion: null,
            tiquet: String(toEdit.tiquet),
            fechaViaje: new Date(toEdit.fechaViaje).toISOString().slice(0, 10),
            chofer: chofer,
            chapa: toEdit.chapa,
            producto: toEdit.producto,
            origen: toEdit.origen,
            destino: toEdit.destino,
            precio: Number(toEdit.precio).toFixed(2),
            precioLiquidacion: Number(toEdit.precioLiquidacion).toFixed(2),
            kgOrigen: String(toEdit.kgOrigen),
            kgDestino: String(toEdit.kgDestino),
                            ...
                            {formData.id ? (
                                <label>
                                    ...
                                    <Select.Root
                                        ...
                                        onValueChange={(v) =>
                                            setFormData({ ...formData, fechaLiquidacion: v })
                                        }
                                    >
                                        <Select.Trigger />
