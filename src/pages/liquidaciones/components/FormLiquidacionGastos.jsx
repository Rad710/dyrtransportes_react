...
function FormLiquidacionGastos({
    ...
    const handleSubmit = async (e) => {
        ...
        for (const field in formData) {
            if (
                field != "razon" &&
                (formData[field] === "" || (formData[field] === null && select === "conBoleta"))
            ) {
        ...
        if (!response?.response && !response?.message) {
            ...
            const conBoleta = await resultGastos
                ?.filter((gasto) => gasto.boleta !== null)
                ?.map((gasto) => ({ ...gasto, checked: false }));
            const sinBoleta = await resultGastos
                ?.filter((gasto) => gasto.boleta === null)
                ?.map((gasto) => ({ ...gasto, checked: false }));
    ...
    const handleEditar = () => {
        ...
        setFormData({
            id: toEdit.id,
            fecha: new Date(toEdit.fecha).toISOString().slice(0, 10),
            chofer: chofer,
            boleta: toEdit.boleta,
            importe: toEdit.importe,
            fechaLiquidacion: fecha,
            razon: toEdit.razon,
        });
    };
                ...
                <Button
                    ...
                    disabled={
                        liquidacionGastos.conBoleta.filter((entry) => entry.checked).length +
                            liquidacionGastos.sinBoleta.filter((entry) => entry.checked).length !==
                        1
                    }
                            ...
                            <label>
                                ...
                                <Select.Root onValueChange={onValueChange}>
                                    ...
                                    <Select.Content position="popper">
                                        <Select.Group>
                                            <Select.Item value="conBoleta">
                                                Gasto Con Boleta
                                            </Select.Item>
                                        </Select.Group>
                                        <Select.Separator />
                                        <Select.Group>
                                            <Select.Item value="sinBoleta">
                                                Gasto Sin Boleta
                                            </Select.Item>
                                        </Select.Group>
                                    </Select.Content>
