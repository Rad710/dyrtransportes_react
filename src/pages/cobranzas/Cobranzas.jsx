...
function Cobranzas({ title }) {
    ...
    const [formData, setFormData] = useState({
        id: "",
        fechaCreacion: planillaDate.toISOString().slice(0, 10),
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            if (!response?.response && !response?.message) {
                const result = response.map((grupo) => ({
                    ...grupo,
                    viajes: grupo.viajes.map((viaje) => ({ viaje: viaje, checked: false })),
                }));
                setCobranzas(result);
                setLoading(false);
            }
          ...
          ${planillaDate.toLocaleDateString("es-ES", {
              ...
              timeZone: "GMT",
                ...
                <div className="gap-5 md:flex justify-end md:2/3">
                    ...
                    <AlertButton handleDelete={handleDelete} disabled={tracker.length <= 0} />
                </div>
            </div>
